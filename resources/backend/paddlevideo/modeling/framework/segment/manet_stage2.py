...
from resources.backend.paddlevideo.loader import build_dataloader, build_dataset, RandomIdentitySampler
from resources.backend.paddlevideo.loader.builder import build_custom_dataloader, build_sampler, build_pipeline
from resources.backend.paddlevideo.solver import build_lr, build_optimizer
from resources.backend.paddlevideo.utils import get_logger, load
import paddle.distributed as dist
import os.path as osp
from resources.backend.paddlevideo.utils import (build_record, log_batch, log_epoch, save, mkdir)
...
from resources.backend.paddlevideo.utils.config import AttrDict
from resources.backend.paddlevideo.utils.manet_utils import float_, _palette, damage_masks, int_, long_, label2colormap, mask_damager, \
    rough_ROI
from resources.backend.paddlevideo.utils import manet_utils
...
class ManetSegment_Stage2(BaseSegment):
    def train_step(self,
        ...
        valid_num_workers = cfg['DATASET'].get('valid_num_workers',
                                               num_workers)
        ...
        while step < max_iters:
            ...
            for r in range(round_):
                ...
                if cfg.get('train_inter_use_true_result'):
                    if r != round_ - 1:
                        if r == 0:
    ...
    def test_step(self, weights, parallel=False, **cfg):
        # 1. Construct model.
        ...
        seqs = []
        with open(
                os.path.join(cfg['DATASET'].test.file_path, 'ImageSets',
                             '2017', 'val' + '.txt')) as f:
