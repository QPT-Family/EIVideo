...
from EIVideo.eivideo.utils.manet.manet_utils import int_, float_, long_, load
from EIVideo.eivideo.utils.manet.manet_utils import kaiming_normal_
...
class IntVOS(nn.Layer):
    def __init__(self, feature_extracter, **cfg):
        ...
        cfg['train_bn_mom'] = float(cfg['train_bn_mom'])
        cfg['model_aspp_outdim'] = int(cfg['model_aspp_outdim'])
        cfg['model_semantic_embedding_dim'] = int(cfg['model_semantic_embedding_dim'])
        cfg['model_head_embedding_dim'] = int(cfg['model_head_embedding_dim'])
        cfg['model_max_local_distance'] = int(cfg['model_max_local_distance'])
        cfg['model_useintseg'] = bool(False) if cfg['model_useintseg'] == 'False' else bool(True)
        cfg['test_mode'] = bool(False) if cfg['test_mode'] == 'False' else bool(True)
