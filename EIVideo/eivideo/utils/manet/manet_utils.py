...
def _no_grad_trunc_normal_(tensor, mean, std, a, b):
    ...
    with paddle.no_grad():
        ...
        tensor.set_value(tensor.multiply(paddle.to_tensor(std *
                                                          math.sqrt(2.))))
        tensor.add_(mean)
...
class Resize_manet(object):
    ...
    def __call__(self, results):
        ...
        tmp = cv2.resize(tmp, dsize=(new_w, new_h), interpolation=cv2.INTER_CUBIC)
        results = tmp
        return results
...
class ToTensor_manet(object):
    ...
    def __call__(self, results):
        from EIVideo.eivideo.utils.config import get_config
        from EIVideo import CONFIG_FILE_PATH
        cfg = get_config(CONFIG_FILE_PATH)
        outputH = cfg['TEST']['output_high']
        outputW = cfg['TEST']['output_width']
        tmp = results
        if tmp.ndim == 2:
            tmp = tmp[:, :, np.newaxis]
        else:
        ...
        tmp = tmp.reshape([1, 3, int(outputH), int(outputW)])
        results = paddle.to_tensor(tmp).astype('float32')
        return results


class ToTensor_manet2(object):
    """Convert ndarrays in results to Tensors."""
    def __call__(self, results):
